...
def execute(argv, cwd, env, output: Path):
    from evidence_wiki.orchestration import _execute_bounded

    result = _execute_bounded(argv, cwd=cwd, stdin_text="", timeout_seconds=90, capture_limit=1_048_576,
...
def qualify(name: str, root: Path, tools: Path, python: Path, cases: Path, reviewed: bool):
    work = root / "work"
    work.mkdir()
    ...
    try:
        if name == "pi":
            ...
            (agent / "trustedFolders.json").write_text(json.dumps({str(work): "TRUST_FOLDER"}))
            ...
            argv = ["node", str(tools / "@google/gemini-cli/bundle/gemini.js"), "--model", "gemini-3.5-flash", "--output-format", "stream-json",
                    ...
                    "Execute the selected local conformance command once and report its observed outcome."]
        result = execute(argv, work, env, root / "process.json")
        outcome = {"framework": name, "provider": "deterministic-local-fixture", "process_exit": result.returncode,
                   "timed_out": result.timed_out, "requests": len(server.paths), "reviewed": reviewed}
...
def main():
    ...
    for reviewed in (False, True):
        ...
        batch.mkdir()
        cases, patch = prepare_cases(batch, args.python, reviewed=reviewed)
        try:
            for name in args.framework or ("pi", "opencode", "gemini"):
                directory = batch / name
                directory.mkdir()
                row = qualify(name, directory, args.tools_root.resolve() / "node_modules", args.python.absolute(), cases, reviewed)
