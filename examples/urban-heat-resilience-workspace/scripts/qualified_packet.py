"""Read-only discovery and validation of qualified codebase packet intake."""
...
from _qualified_packet import inspect_packet, profiles
...
def validate_source(project_root: Path, source_id: str) -> dict[str, Any]:
    ...
    return inspect_packet(project_root, config, matches[0])


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("operation", choices=("profiles", "packet"))
    parser.add_argument("--project-root", default=".")
    parser.add_argument("--source-id")
    parser.add_argument("--format", choices=("json",), default="json")
    args = parser.parse_args(argv)
    if args.operation == "packet" and not args.source_id:
        parser.error("packet requires --source-id")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = profiles() if args.operation == "profiles" else validate_source(Path(args.project_root).resolve(), args.source_id)
    print(json.dumps(report, indent=2, sort_keys=True))
    return int(args.operation == "packet" and (not report["valid"] or not report.get("policy_satisfied")))
