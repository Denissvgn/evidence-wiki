...
| `table` | `.csv`, `.tsv`, `.xlsx`, `.xls`, `.parquet`, `.feather`, `.jsonl` |
...
`source_inventory.py --report` prints a Markdown report to stdout instead of JSONL dry-run output. The report includes:
