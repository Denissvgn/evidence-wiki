...
- `raw.source_roots`: non-empty list of immutable evidence roots to create and scan.
...
- `outputs.supported_formats`: non-empty list of expected output formats.
...
The same sections may also be nested under `research_yml`. Dictionaries are deep-merged; list values replace the starter list. Unknown nested keys are allowed so domain packs and future integrations can extend the contract, but unknown setup-profile top-level keys are refused.
...
The initializer creates the configured `output_dir` when the integration is enabled. It records the command but does not run it. Generated architecture artifacts remain under `sources/` and become source evidence; maintained interpretation still belongs in `wiki/` after citation and review.
