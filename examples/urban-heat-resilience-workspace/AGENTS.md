...
3. Answer: work the highest-priority actionable questions with the `research-answer` skill, which uses the query workflow and updates each question's lifecycle (`in_progress`, then `answered`, `blocked`, `deferred`, or `rejected`). An `answered` question links a real `answer_page` and cites `source_ids`.
