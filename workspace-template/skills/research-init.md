...
Pass this profile to `evidence-wiki init` with `--profile`, or to `scripts/init_research_workspace.py` when working from a source checkout. Use `--dry-run` when the inferred decisions need review before writes. Keep the profile reviewable so future agents can replay or audit the setup. Treat the profile path and target path as trusted operator inputs; when a parent agent prepares a reviewed setup bundle, keep the profile and intended target under one directory and pass `--scope-root PATH` so initialization refuses to read or write outside that root.
