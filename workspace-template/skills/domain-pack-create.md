...
- keep required coverage facets autonomously satisfiable: do not use a manual-only policy such as `manual_review_required`, `domain_pack_allowed`, `manual_review`, or a pack-declared source/freshness/identity policy in `required_facets` unless the pack explicitly declares `domain_pack.human_gated: true`. A pack-declared policy that carries a rule under `domain_pack.policy_rules` and has neither `manual_review_required` nor a leaf with `when_absent: manual_review` is deterministic and is exempt from that restriction — the package decides it, so a required facet may use it directly;
- declare `domain_pack.policy_rules` only for the part of a pack policy that is genuinely mechanical. Write the `policy_vocabularies` definition text first — the sentence a reviewer would act on — then add a rule when a machine can settle that sentence from the source's structured-view sidecar, its delivery provenance, or the question's frontmatter. A rule is data, never code: the primitives are `max_age`, `equals`, `numeric_range`, `regex`, and `one_of_provenance`, composed with `all_of` and `any_of`. Each rule key must be a policy the same pack declares under `policy_vocabularies.source_policy`, `.freshness_policy`, or `.identity_policy`, namespaced to this pack's own `domain_pack.name`;
- keep judgement calls manual. `pack:general-science/study-recency` asks a reviewer to confirm that study dates, dataset releases, and follow-up literature are recent enough *for the scientific question*; no threshold is right across studies, so that policy ships with no rule and should stay that way. Do not approximate a judgement with a number nobody agreed on. Where a policy is partly mechanical, declare the rule and set `manual_review_required: true`, which runs the checks and still routes to a human;
- use `when_absent: manual_review` only on a `record/...` field whose terminal member is genuinely optional. It applies to `max_age`, `equals`, `numeric_range`, and `regex`, never provenance, question operands, compositions, or `one_of_provenance`. Require the normalizer to emit every mapping parent and omit only the known-absent terminal member; null, arrays, missing parents, and corrupt structured evidence remain hard failures. A required facet using this route requires `domain_pack.human_gated: true`;
