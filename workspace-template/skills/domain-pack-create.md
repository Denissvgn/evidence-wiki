...
- keep required coverage facets autonomously satisfiable: do not use a manual-only policy such as `manual_review_required`, `domain_pack_allowed`, `manual_review`, or a pack-declared source/freshness/identity policy in `required_facets` unless the pack explicitly declares `domain_pack.human_gated: true`. A pack-declared policy that carries a rule under `domain_pack.policy_rules` and does not set `manual_review_required` is deterministic and is exempt from that restriction — the package decides it, so a required facet may use it directly;
