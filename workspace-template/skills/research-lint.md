...
- `question_unresolved`, `question_answer_missing`, `question_blocked_reason`, and `question_answer_ungrounded`: keep question task records honest. An `answered` question must link a resolvable `answer_page` and cite `source_ids` on the question or its answer page; a `blocked` question must record a `blocked_reason`.
- Publication readiness also reports standards no-ship causes from coverage policy results: withdrawn or superseded standards (`standard_status_withdrawn`, `standard_status_superseded`), draft status, missing edition/year, missing registry terms, stale registry metadata, missing OJEU/harmonised references, and guidance used as legal authority.
