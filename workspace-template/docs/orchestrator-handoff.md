...
| `RUN_EVENTS_INVALID` | `runs/<run_id>/events.jsonl` has a malformed line, a record without a string `event_id`, or duplicate event ids. | Preserve the event log, restore a verified copy or repair the reported line or duplicate id, then run `run_controller.py recover` before the next mutation. |
...
| `RUN_MUTATION_WRITE_FAILED` | A run-state, event-log, ledger, or quarantine write failed at the reported boundary. | Restore write access or free space for the run directory, retain any generated `.tmp` artifact, then run `run_controller.py recover` before retrying the mutation. |
| `RUN_PENDING_EVENT_INVALID` | `recover` found a `_pending_event` journal that is not an object with a string `event_id`. | Preserve `runs/<run_id>/run-state.json` for audit and restore a verified copy whose `_pending_event` has that shape before rerunning `recover`. |
