...
| `ORCHESTRATION_LEGACY_ACTION_UNBOUND` | A pre-0.2.1 pending action has not yet been bound to a controller-owned trusted static-input fingerprint. | Replay the pending action with `evidence-wiki orchestrate next --resume`, or use managed `evidence-wiki orchestrate resume`, before submitting a result. The replay binds a controller-owned fingerprint before any worker is launched. |
