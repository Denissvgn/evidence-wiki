...
Use `--profile <path>` when an agent has prepared a setup profile. See `docs/workspace-initialization.md` for CLI usage and `docs/workspace-init-profile.md` for the profile schema. Review the profile or run `--dry-run` before writing workspace files when the inferred decisions need human confirmation. Treat the profile path and target path as trusted operator inputs; for parent-agent handoffs, keep the reviewed profile and intended target under one directory and pass `--scope-root <path>` so initialization refuses paths outside that root. Profile-driven initialization also writes `docs/workspace-init-report.md` unless the profile sets a different workspace-relative `init_report.path`.
