...
| `table` | `.csv`, `.tsv`, `.xlsx`, `.xls`, `.parquet`, `.feather` — only `.csv`/`.tsv` are normalized; the rest stay classified-only |
| `structured_data` | `.json`, `.jsonl` — structured payloads whose shape is fields rather than prose or rows. Classified-only unless the workspace configures a normalization adapter for the kind (see [research-yml.md](research-yml.md)). `.jsonl` classified as `table` before this kind existed; nothing normalized it as one, so re-running inventory relabels it without changing its source id |
