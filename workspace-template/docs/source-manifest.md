...
- `raw_fingerprint`: `sha256:`-prefixed content hash of the raw inputs for `paper`, `pdf`, `html`, CSV/TSV `table`, and `structured_data` records (source files plus any paired PDF, provenance sidecars, and the companion files a sidecar declared). Normalization re-generates a record only when this value changes, enabling incremental re-normalization. Omitted for kinds whose normalized output is not derived from raw bytes (links, codebase records).
